...
.constant('DEFAULT_INVOICE', {
  ...
  company_info: {
    name: 'SRI OM SAKTHI TRADERS',
    web_link: 'www.sostraders.com',
    address1: 'no,142 ph road',
    address2: 'jb estate,Avadi',
    postal: 'Chennai-54'
  },
  items:[
...
.controller('InvoiceCtrl', ['$scope', '$http', 'DEFAULT_INVOICE', 'DEFAULT_LOGO', 'LocalStorage', 'Currency',
  ...
  // Calculates the sgst tax of the invoice
  $scope.calculateTaxsgst = function() {
    return (($scope.invoice.sgst * $scope.invoiceSubTotal())/100);
  };

    // Calculates the cgst tax of the invoice
    $scope.calculateTaxcgst = function() {
  ...
  $scope.calculateGrandTotal = function() {
    saveInvoice();
    return $scope.calculateTaxsgst() + $scope.calculateTaxcgst() + $scope.invoiceSubTotal();
  };
